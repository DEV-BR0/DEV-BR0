...
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=21&duration=2800&pause=900&color=6366F1&center=true&vCenter=true&width=750&lines=React+%2B+Next.js+Developer;Svelte+%2B+SvelteKit+Enthusiast;Node.js+%2B+Express+Backend;REST+API+%26+Redux+Architect;Animating+the+Web+with+GSAP+%26+AOS;Type-Safe+with+TypeScript" alt="Typing SVG" />
...
<img src="https://github-readme-streak-stats-eight.vercel.app?user=DEV-BR0&theme=radical&hide_border=true" alt="GitHub streak stats"/>
...
- Streak stats now run on `github-readme-streak-stats-eight.vercel.app` instead of the original `streak-stats.demolab.com`. The `demolab.com` mirror is known to go down intermittently, and this Vercel mirror is currently the most reliable public alternative.

If a card still doesn't render, wait a few minutes for the cache to refresh, or self-host your own instance on Vercel for guaranteed uptime (see [github-readme-stats](https://github.com/anuraghazra/github-readme-stats#deploy-on-your-own) and [github-readme-streak-stats](https://github.com/DenverCoder1/github-readme-streak-stats#deploying-it-on-your-own) docs).
...
<img src="https://raw.githubusercontent.com/DEV-BR0/DEV-BR0/main/metrics.isocalendar.svg" alt="animated metrics dashboard: isometric 3D commit calendar, activity graph, language breakdown, and achievements" width="100%"/>
...
A single animated dashboard generated by the `.github/workflows/snake.yml` Action (using `lowlighter/metrics`), combining:

- 🧊 **Isometric 3D calendar** — full-year contribution history as an animated isometric grid
- ⚡ **Activity graph** — your last 14 days of commits, PRs, issues, and reviews
- 🥧 **Language breakdown** — animated donut chart of your most-used languages
- 🏆 **Achievements** — animated badges for milestones (stars, PRs, commits, etc.)
- ✍️ **Introduction** — typing-style animated bio line

The Action commits `metrics.isocalendar.svg` directly to the repo root on `main` — no separate branch or output folder needed.

If the image doesn't load:
1. Check the **Actions** tab on `DEV-BR0/DEV-BR0` and confirm the `Isometric Commit Calendar` workflow ran successfully (green checkmark, not red ❌).
2. This dashboard does more work than a simple calendar (language analysis across repos, achievement checks), so a run can take **2–5 minutes** — a yellow/orange spinning icon means it's still working, not stuck.
3. Confirm `metrics.isocalendar.svg` actually appeared in the repo root on `main` after the run — check the repo's file browser.
4. If the workflow hasn't run yet, trigger it manually from **Actions → Isometric Commit Calendar → Run workflow**.
5. `raw.githubusercontent.com` caches files for a few minutes — if you just pushed, wait 5–10 minutes and hard-refresh before assuming it's broken.
