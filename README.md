...
<img src="https://raw.githubusercontent.com/DEV-BR0/DEV-BR0/main/metrics.isocalendar.svg" alt="animated isometric 3D commit calendar" width="100%"/>
...
This renders your yearly contribution history as an animated isometric 3D calendar, generated by the `.github/workflows/snake.yml` Action (using `lowlighter/metrics`, one of the most actively maintained GitHub profile metrics tools). The Action commits `metrics.isocalendar.svg` directly to the repo root on `main` — no separate branch or output folder needed.
...
2. Confirm `metrics.isocalendar.svg` actually appeared in the repo root on `main` after the run — check the repo's file browser.
3. If the workflow hasn't run yet, trigger it manually from **Actions → Isometric Commit Calendar → Run workflow**.
